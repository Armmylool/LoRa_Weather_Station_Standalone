...
uint32_t dataProcess::getMedian32(uint32_t val1, uint32_t val2, uint32_t val3) {
	uint32_t middle ;
	if ((val1 <= val2) && (val1 <= val3)) {
    middle = (val2 <= val3) ? val2 : val3;
  } else if ((val2 <= val1) && (val2 <= val3)) {
...
bool RS485sensor::read (uint8_t sensorType ,uint8_t slaveID, uint16_t address, uint16_t length, Stream* serialPort) {
	...
	uint8_t result;
	uint16_t rawBuffer[readAttempt][16] = {0}; 
	...
	while (startAttempt < readAttempt && retry_FLAGS < maxRetry) {
		...
		if (result == modbus.ku8MBSuccess) {
			for (int cnt = 0; cnt < length ; cnt++) {
				if (cnt < 16) { /* Protect Buffer overflow by the max array */
	...
	if (startAttempt == 0) {
		Serial.println("[ERROR] No successful reads!");
		return false;
	}
	
		if (sensorType == SOIL) {
		memset(&currentSoil, 0, sizeof(soilData));	/* Set the soil struct to zero first*/
		...
		} else if (startAttempt == 2) {
			currentSoil.moisture_content = (rawBuffer[0][0] + rawBuffer[1][0]) / 2;
			currentSoil.soil_Temp        = (rawBuffer[0][1] + rawBuffer[1][1]) / 2;
			currentSoil.EC               = (rawBuffer[0][2] + rawBuffer[1][2]) / 2;
			currentSoil.PH               = (rawBuffer[0][3] + rawBuffer[1][3]) / 2;
			currentSoil.N                = (rawBuffer[0][4] + rawBuffer[1][4]) / 2;
			currentSoil.P                = (rawBuffer[0][5] + rawBuffer[1][5]) / 2;
			currentSoil.K                = (rawBuffer[0][6] + rawBuffer[1][6]) / 2;
		} else {
			currentSoil.moisture_content = rawBuffer[0][0];
			currentSoil.soil_Temp        = rawBuffer[0][1];
			currentSoil.EC               = rawBuffer[0][2];
			currentSoil.PH               = rawBuffer[0][3];
			currentSoil.N                = rawBuffer[0][4];
			currentSoil.P                = rawBuffer[0][5];
			currentSoil.K                = rawBuffer[0][6];
		}
		return true;
	}
	else if (sensorType == WEATHER) {
		memset(&currentWeather, 0, sizeof(weatherData));	/* Set the weather struct to zero first */
		...
		if (startAttempt >= 3) {
			...
			uint32_t lux1 = ((uint32_t)rawBuffer[0][10] << 16) | rawBuffer[0][11];
			uint32_t lux2 = ((uint32_t)rawBuffer[1][10] << 16) | rawBuffer[1][11];
			uint32_t lux3 = ((uint32_t)rawBuffer[2][10] << 16) | rawBuffer[2][11];
			currentWeather.illuminance = postProcessing.getMedian32(lux1, lux2, lux3);
			...
			currentWeather.windSpeed         = (rawBuffer[0][0] + rawBuffer[1][0]) / 2;
			currentWeather.windStrength      = (rawBuffer[0][1] + rawBuffer[1][1]) / 2;
			currentWeather. WindDirection_Num = (rawBuffer[0][2] + rawBuffer[1][2]) / 2;
			currentWeather. windDirection_Deg = (rawBuffer[0][3] + rawBuffer[1][3]) / 2;
			currentWeather. humidity          = (rawBuffer[0][4] + rawBuffer[1][4]) / 2;
			currentWeather.temperature       = (rawBuffer[0][5] + rawBuffer[1][5]) / 2;
			currentWeather.noise             = (rawBuffer[0][6] + rawBuffer[1][6]) / 2;
			currentWeather.PM_2_5            = (rawBuffer[0][7] + rawBuffer[1][7]) / 2;
			currentWeather. PM_10             = (rawBuffer[0][8] + rawBuffer[1][8]) / 2;
			currentWeather.pressure          = (rawBuffer[0][9] + rawBuffer[1][9]) / 2;
			
			uint32_t lux1 = ((uint32_t)rawBuffer[0][10] << 16) | rawBuffer[0][11];
			uint32_t lux2 = ((uint32_t)rawBuffer[1][10] << 16) | rawBuffer[1][11];
			currentWeather.illuminance = (lux1 + lux2) / 2;
			
			currentWeather.rainfall         = (rawBuffer[0][13] + rawBuffer[1][13]) / 2;
			currentWeather.solar_irradiance = (rawBuffer[0][15] + rawBuffer[1][15]) / 2;
			
		} else {
			currentWeather.windSpeed         = rawBuffer[0][0];
			currentWeather.windStrength      = rawBuffer[0][1];
			currentWeather.WindDirection_Num = rawBuffer[0][2];
			currentWeather.windDirection_Deg = rawBuffer[0][3];
			currentWeather.humidity          = rawBuffer[0][4];
			currentWeather.temperature       = rawBuffer[0][5];
			currentWeather.noise             = rawBuffer[0][6];
			currentWeather.PM_2_5            = rawBuffer[0][7];
			currentWeather. PM_10             = rawBuffer[0][8];
			currentWeather.pressure          = rawBuffer[0][9];
			
			currentWeather.illuminance = ((uint32_t)rawBuffer[0][10] << 16) | rawBuffer[0][11];
			currentWeather.rainfall         = rawBuffer[0][13];
			currentWeather.solar_irradiance = rawBuffer[0][15];
		}
		return true ;
	}
	return false ;
...
bool timeSensor::getTime(timeStruct *val) {
	...
	Serial.print(val->dateStr) ;
	Serial.print(" ");
	Serial.println(val->timeStr) ;
	delay(50) ;
	return true ;
}