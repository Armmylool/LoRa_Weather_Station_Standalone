...
bool RS485sensor::read(uint8_t sensorType, uint8_t slaveID, uint16_t address, uint16_t length, Stream* serialPort) {
  ...
  /* Flush any stale data from the serial RX buffer and settle */
  ...
  if (sensorType == SOIL) {
    ...
    currentSensor.soil_ec = postProcessing.getMedian(buf, startAttempt);
    ...
    currentSensor.soil_N = postProcessing.getMedian(buf, startAttempt);

    for (uint8_t i = 0; i < startAttempt; i++) buf[i] = rawBuffer[i][5];
    currentSensor.soil_P = postProcessing.getMedian(buf, startAttempt);

    for (uint8_t i = 0; i < startAttempt; i++) buf[i] = rawBuffer[i][6];
    currentSensor.soil_K = postProcessing.getMedian(buf, startAttempt);

    /* Zero-retry: if key values are zero, re-read like WEATHER */
    uint8_t zeroRetry = 0;
    while ((currentSensor.soil_humi == 0 ||
            currentSensor.soil_temp == 0 ||
            currentSensor.soil_ec == 0) && zeroRetry < maxRetry) {
      esp_task_wdt_reset();
      zeroRetry++;
      Serial.print("[WARN] Soil humi/temp/EC zero, retry #");
      ...
      /* Re-init modbus and flush stale data before retrying */
      while (serialPort->available()) { serialPort->read(); }
      serialPort->flush();
      delay(200);
      modbus.begin(slaveID, *serialPort);
      ...
      for (uint8_t i = 0; i < startAttempt; i++) buf[i] = rawBuffer[i][3];
      uint16_t ph_med = postProcessing.getMedian(buf, startAttempt);
      if (ph_med > 255) ph_med = 255;
      currentSensor.soil_ph = (uint8_t)ph_med;
    ...
    if (currentSensor.soil_humi == 0 ||
        currentSensor.soil_temp == 0 ||
        currentSensor.soil_ec == 0) {
      Serial.println("[ERROR] Soil humi/temp/EC still zero after retries");
    }
    ...
    for (uint8_t i = 0; i < startAttempt; i++) buf[i] = rawBuffer[i][13];
    currentSensor.rainfall = postProcessing.getMedian(buf, startAttempt);
